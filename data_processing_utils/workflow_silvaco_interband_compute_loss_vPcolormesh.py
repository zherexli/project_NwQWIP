...
parser.add_argument("-p","--apply_peak_regulation",action='store_true',default=False,help="Instruct the program if it needs to apply extra loss function to match the peak wavelength.")
#NOTE: So far, for the interband PL fitting, I find that it doesn't need to apply the extra peak-matching loss function.
args=parser.parse_args()

# load experimental data
data_exp_src = np.genfromtxt("src_pl_spec_InGaAs_Temperature_300K.txt",skip_header=1)
wavelength_exp = data_exp_src[:,0]/1000.0 # unit: um
data_exp = data_exp_src[:,1]/data_exp_src[:,1].max() # normalise the experimental data since simulated data was already normalised.
...
if args.load_processed_data==False:
    ...
    for ii in np.arange(data_sim.shape[0]):
        ...
        if slope_data_sim_temp.max()*slope_data_sim_temp.min()>=0:
            ...
            if args.apply_peak_regulation==True:
...
if os.path.exists(output_file_name) and args.load_processed_data==True:
    ...
    ax2.set_title(f"BestFit: xGa={data_sim[fit_best_idx,0]}; dEcPer={data_sim[fit_best_idx,1]}",fontsize=16)
